...
def checkout():
    ...
    # Query selected items using mysql.connector
    ...
    # Retrieve item details for selected items
    format_strings = ','.join(['%s'] * len(selected_item_ids))
    query = f"""
        SELECT id, name, price, grid_image
        FROM items
        WHERE id IN ({format_strings})
    """
    cursor.execute(query, tuple(selected_item_ids))  # Pass the IDs as a tuple
    ...
    return render_template('checkout.html', cart_items=selected_items, total_price=total_price)
